    ...
    Init: function (elem) {
        ...
        function rebuildAutocomplete() {
            ...
            if (searchType || rectrictedCountry) {
                ...
                options = {};
                if (searchType) options.types = searchType.split(',');
                if (rectrictedCountry) options.componentRestrictions = { country: rectrictedCountry };
            }
        ...
        Aspectize.UiExtensions.AddMergedPropertyChangeObserver(elem, function (sender, arg) {
            ...
            if (arg.SearchType)   rebuildAutocomplete();
            if (arg.RestrictToCountry) autocomplete.setComponentRestrictions({ country: arg.RestrictToCountry });
        });
    }
});
